...
def get_accuracy_rate_by_category(file_ind):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            sql_statement = "SELECT checked, count(*) as count, SUM(clear_count) as sum_clear, SUM(fail_count) as sum_fail, ( SUM(clear_count) / ( SUM(clear_count) + SUM(fail_count) ) ) as accuracy_rate FROM {0} where checked = 1 group by checked;".format(table+"_view")
            cursor.execute(sql_statement)
