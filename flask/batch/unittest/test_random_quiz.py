...
class TestRandomQuiz(unittest.TestCase):
    ...
    def test_min_rate(self):
        ...
        with conn.cursor() as cursor:
            # 正解数操作
            # 20%
            sql = "UPDATE {0} SET clear_count = {1}, fail_count = {2} WHERE quiz_num = {3} ".format(table,2,8,1)
            cursor.execute(sql)
            # 40%
            sql = "UPDATE {0} SET clear_count = {1}, fail_count = {2} WHERE quiz_num = {3} ".format(table,4,6,2)
            cursor.execute(sql)
            # 60%
            sql = "UPDATE {0} SET clear_count = {1}, fail_count = {2} WHERE quiz_num = {3} ".format(table,6,4,3)
            cursor.execute(sql)
            # 80%
            sql = "UPDATE {0} SET clear_count = {1}, fail_count = {2} WHERE quiz_num = {3} ".format(table,8,2,4)
            cursor.execute(sql)
            # 100%
            sql = "UPDATE {0} SET clear_count = {1}, fail_count = {2} WHERE quiz_num = {3} ".format(table,10,0,5)
            cursor.execute(sql)
            # コミット
            conn.commit()
